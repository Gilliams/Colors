...
function randomColors() {
  ...
  colorDivs.forEach((div, index) => {
    ...
    if (div.classList.contains("locked")) {
      initialColors.push(hexText.getBoundingClientRect);
      return;
    } else {
      // Add it to the array
  ...
  adjustButton.forEach((button, index) => {
    ...
    checkTextConstrast(initialColors[index], lockButton[index]);
  });
}
...
function savePalettes(e) {
  ...
  if (paletteObjects) {
    paletteNb = paletteObjects.length;
  } else {
    paletteNb = savePalettes.length;
  }
